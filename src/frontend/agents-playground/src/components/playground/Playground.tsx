...
const DRIVER_PHONE =
  process.env.NEXT_PUBLIC_TEST_DRIVER_PHONE ?? "84867347452";

function generateDriverRoomName() {
  return `driver-${DRIVER_PHONE.replace(/^\+/, "")}-${Math.floor(Date.now() / 1000)}`;
}
...
export default function Playground({
  ...
  const [tokenFetchOptions, setTokenFetchOptions] =
    useState<TokenSourceFetchOptions>(() => {
      ...
      const opts: TokenSourceFetchOptions = {
        roomName: generateDriverRoomName(),
      };
      if (initialAgentOptions) {
        opts.agentName = initialAgentOptions.agentName ?? "driver-assistant-local";
        opts.agentMetadata = initialAgentOptions.metadata ?? "";
      } else {
        opts.agentName = "driver-assistant-local";
      }
      return opts;
    });
  ...
  // Config loads async — keep agent name + driver room in sync before connect
  useEffect(() => {
    if (connectionState !== ConnectionState.Disconnected) return;
    const agentName =
      config.settings.agent ??
      (typeof config.agent_dispatch === "object" &&
      config.agent_dispatch &&
      "agentName" in config.agent_dispatch
        ? String(config.agent_dispatch.agentName)
        : undefined) ??
      "driver-assistant-local";
    setTokenFetchOptions((prev) => ({
      ...prev,
      agentName: prev.agentName || agentName,
      roomName:
        prev.roomName && !/^room-[a-z0-9]+$/i.test(prev.roomName)
          ? prev.roomName
          : generateDriverRoomName(),
    }));
          ...
          <div className="flex flex-col gap-2">
            <EditableNameValueRow
              ...
              onValueChange={(value) => {
                ...
                setTokenFetchOptions((prev) => ({
                  ...prev,
                  roomName: value || generateDriverRoomName(),
                }));
              }}
              placeholder="Auto"
